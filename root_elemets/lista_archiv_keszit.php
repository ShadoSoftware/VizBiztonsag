<?php
				  ...
				  $uzem_id=$_POST["uzemID"];
					...
					$kiiras='<tr><th width="25"></th><th>VBT elnevezése</th><th>Üzemeltető megnevezése</th><th>Vízellátó rendszer megnevezése</th><th>Létrehozva</th><th>Utolsó módosítás</th><th>Visszaállítás!</th></tr><tbody>';
				 
				   $sql_text1= 'SELECT * FROM `vbt`  WHERE `archiv`=0 ORDER BY modositva DESC';
				   $sql_text2= "SELECT * FROM `vbt`  WHERE `archiv`=0 AND `UZEMELTETO_ID`='$uzem_id'  ORDER BY modositva DESC";
				...
				foreach($json as $id => $item) {
						...
						$kiiras.='<tr id="row_id_'.$item["ID"].'"><td width="25"><input type="checkbox" id="checkall"></td><td>'.$item["vbt_elnevezes"].'</td><td>'.$item["vizmu_neve"].'</td><td>'.$item["vizellato_megnevezes"].'</td><td>'.$datum1.'</td><td>'.$datum2.'</td><td><button type="button" id="'.$item["ID"].'" class="btn btn-danger btn-xs delete" onclick="aktival('.$item["ID"].')">Visszaállít</button></td></tr>';
					} 
